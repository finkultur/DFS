#!/bin/bash
# Runs some CPU2006 tests with the test input data.

start429=$(date +%s)
echo $start429
echo "Starting test 429.milc"
cd /opt/benchmarks/SPEC2006/benchspec/CPU2006/429.mcf/
/opt/tilepro/bin/tile-monitor --pci --tile 1x1 --here -- run/build_base_compsys.0000/mcf data/test/input/inp.in
end429=$(date +%s)
...
/opt/tilepro/bin/tile-monitor --pci --tile 1x1 --here -- run/build_base_compsys.0000/milc data/test/input/su3imp.in
end433=$(date +%s)
...
/opt/tilepro/bin/tile-monitor --pci --tile 1x1 --here -- run/build_base_compsys.0000/soplex data/test/input/test.mps
end450=$(date +%s)
...
/opt/tilepro/bin/tile-monitor --pci --tile 1x1 --here -- run/build_base_compsys.0000/lbm 20 myreference.dat 0 1 data/test/input/100_100_130_cf_a.of 
end470=$(date +%s)
